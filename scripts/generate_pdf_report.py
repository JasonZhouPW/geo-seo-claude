...
def draw_page2(c, data):
    ...
    # Update y to account for 3 rows of platform cards (28 high + 6 gap = 34 per row)
    y -= (card_h + 6) * 3
...
def draw_page5(c, data):
    ...
    c.saveState()
    c.setFont("Helvetica-Bold", 9)
    title_text = "Methodology & Standards"
    c.restoreState()
    ...
    standards_text = "llms.txt emerging standard, RSL 1.0 licensing framework"
    max_w = W - 44*mm

    # Count total lines
    total_lines = 1 + len(method_lines)  # title + method_lines
    # Add wrapped standards lines
    c2 = canvas.Canvas("/dev/null", pagesize=A4)
    c2.setFont("Helvetica", 7.5)
    wrapped_standards = word_wrap_text(c2, standards_text, max_w, "Helvetica", 7.5)
    total_lines += len(wrapped_standards)

    # Box height: 8 for title area + 10 per content line + 8 padding
    box_h = 8 + (total_lines * 10) + 10
    box_y = y - box_h
    draw_rect(c, 18*mm, box_y, W - 36*mm, box_h, fill=LIGHT_BG, radius=6, stroke=BORDER, lw=0.6)

    c.saveState()
    c.setFont("Helvetica-Bold", 9)
    c.setFillColor(NAVY)
    c.drawString(22*mm, y - 4, title_text)
    c.setFont("Helvetica", 7.5)
    c.setFillColor(TEXT_MID)
    my = y - 16
    ...
    max_w = W - 36*mm
    wrapped_disclaimer = word_wrap_text(c, disclaimer, max_w, "Helvetica", 6.5)
    dy = y
    for ln in wrapped_disclaimer:
        c.drawString(18*mm, dy, ln)
        dy -= 9
    c.restoreState()
